 ...
 * for options that don't want tvd to enter its main loop, such as
 * 'h'.
 */
...
parse_opt_h(void)
{
  printf("Usage: tvd [OPTION]\n"
         "Wraps Android TV drivers behind a networking protocol\n"
         "\n"
         "General options:\n"
 ...
 * this as tvd having stopped, while it's actually running in
 * the child process.
 *
 * Next, tvd starts its main loop and opens a socket connection
 ...
 * tvd will wake up when commands or data are available.
 *
 ...
 * the case of tvd, its the options structure with the program's
 * configuration.
 *
...
{
  static const char DEFAULT_SOCKET_NAME[] = "tvd";
